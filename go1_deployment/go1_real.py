...
class UnitreeA1Real:
    ...
    def wait_untill_ros_working(self):
        ...
        while not hasattr(self, "low_state_buffer"):
            rate.sleep()
        self.start_footForce = list(self.low_state_buffer.footForce) # autor:kh
    ...
    def clip_action_before_scale(self, actions):
        actions = torch.clip(actions, -self.clip_actions, self.clip_actions)  # 将action限制在 +- clip_action 中
   ...
   # 限制扭矩大小
    ...
    def compute_contact_filt(self):
        footForce = self.low_state_buffer.footForce
        contact_filt = np.array([1 if self.start_footForce[i] - footForce[i] > 70  else 0 for i in range(4)])
        return contact_filt
    def _get_proprioception_obs(self):
        """
            obs_buf = torch.cat((#skill_vector, 
                            ...
                            0*self.commands[:, 0:2],  #(x,y,yaw,heading)
                            self.commands[:, 0:1],  #[1,1] #(x)
        ...
        ang_vel = 0.25  #1.
        quaternion =  torch.tensor(self.low_state_buffer.imu.quaternion).unsqueeze(0)
        roll, pitch, yaw = self.euler_from_quaternion(quaternion)   #base_quat ?
        imu_obs = torch.stack((roll, pitch), dim=1)
         
        self.dof_pos = dof_pos = torch.tensor(
            self.low_state_buffer.motorState.q , dtype= torch.float32, device= self.model_device).unsqueeze(0)  #debug
        ...
        contact_filt = self.compute_contact_filt()  
        contact_filt = torch.tensors( contact_filt.float()-0.5,device= self.model_device)
        dof_pos_cfg = 1.0
        dof_vel_cfg =  0.05
        zero =  torch.zeros(1,device= self.model_device)
        one_1 = torch.ones(1,device= self.model_device)
        zero_2 = torch.zeros(2,device= self.model_device)
        ...
        obs = torch.cat([
            ...
            zero,
            zero_2,
            self.command_buf[:,0:1],
            one_1 ,
            zero,
            (dof_pos - self.default_dof_pos) * dof_pos_cfg,
            dof_vel * dof_vel_cfg,
            self.action,
            contact_filt 
         ],dim=-1)
        print("========== obs computed ===========",obs.shape)
        return obs
    ...
    def compute_observation(self):
        """ use the u
        pdated low_state_buffer to compute observation vector """
