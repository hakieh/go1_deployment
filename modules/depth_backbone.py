...
class RecurrentDepthBackbone(nn.Module):
    def __init__(self, base_backbone, n_proprio) -> None:
        super().__init__()
        activation = nn.ELU()
...
class depth_inference(DepthOnlyFCBackbone58x87,RecurrentDepthBackbone):
    def __init__(self,cfg):
        self.n_proprio = 3 + 2 + 3 + 4 + 36 + 5
        self.scan_encoder_dims = [128, 64, 32], 
        self.hidden_dims = 512,
    
        self.depth_backbone = DepthOnlyFCBackbone58x87(self.n_proprio, self.scan_encoder_dims[-1], self.hidden_dims)
        self.depth_encoder = RecurrentDepthBackbone(self.depth_backbone, self.n_proprio).to(self.device)
