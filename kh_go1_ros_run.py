...
from go1_real import UnitreeA1Real, resize2d
...
class SkilledA1Real(UnitreeA1Real):
    ...
    def update_low_state(self, ros_msg):
        ...
        if self.move_by_wireless_remote and ros_msg.wirelessRemote.ry > self.skill_mode_threhold:
            ...
            return
            print("================= skill_vel", skill_vel)
        return super().update_low_state(ros_msg)

# ---------------visual encoder-----------------
def get_encoder_script():
    # with open(osp.join(logdir, "config.json"), "r") as f:
    #     config_dict = json.load(f, object_pairs_hook= OrderedDict)

    model_device = torch.device("cuda")
    n_proprio = 3+2+3+4+36+5
    depth_backbone = DepthOnlyFCBackbone58x87(n_proprio, 
                                            32, 
                                            512,
                                            )
    depth_encoder = RecurrentDepthBackbone(depth_backbone,n_proprio).to("cpu")
    path = "./weight/model_36000.pt"
    loaded  = torch.load(path,map_location="cpu")
    depth_encoder.load_state_dict(loaded['depth_encoder_state_dict'])
    depth_encoder.to(torch.device("cuda"))
    depth_encoder.eval()
    rospy.loginfo("------------loading visual model successfully----------------")
...
def get_actor(cfg):
    ...
    n_priv = 3+3+3
    num_obs = n_proprio + 132 + 10*n_proprio + n_priv_latent + n_priv
    history_len = 10
    kwargs = cfg["policy_go1"]
    print("Using MLP and Priviliged Env encoder ActorCritic structure")
    actor_critic: ActorCriticRMA = ActorCriticRMA(n_proprio,
                                                    132,
                                                    num_obs,
                                                    n_priv_latent,
                                                    n_priv,
    ...
    depth_actor = actor_critic.actor
    depth_actor.to(torch.device("cuda"))
    depth_actor.eval()
...
def main(args):
    ...
    # rospy.loginfo("loading model...", config_dict["runner"]["policy_class_name"])
    # unitree_real_env.test()
    # model = getattr(modules, config_dict["runner"]["policy_class_name"])(
    ...
    depth_encoder = get_encoder_script()
    # print("============= loading depth encoder successful")ct
    depth_actor = get_actor(config_dict) 
    # print("============= loading action encoder successful")
    ...
    # =============== model testing 
        # image = unitree_real_env.test().unsqueeze(0).to("cuda")
        image = torch.ones(58,87).cuda() -0.5
        
        obs = unitree_real_env._get_proprioception_kh_obs().to(torch.float).to("cuda")
        depth_latent_yaw = depth_encoder(image, obs[:,:53]).to("cuda")
        depth_latent = depth_latent_yaw[:,:-2]
        yaw = depth_latent_yaw[:,-2:]
        obs[:,6:8] = 1.5*yaw
        actions_output = depth_actor(obs, hist_encoding=True, scandots_latent= depth_latent)
        # print( "=================== action computed 1!================")

# ====================== testing obs kh  ===================

        # obs = unitree_real_env.get_obs()
        # print("=============geting osb successfully",obs.shape)
...
# ====================== testing obs kh  ===================
        ...
        with torch.no_grad():
            ...
            while not rospy.is_shutdown():
                # print("=========footForce===========",unitree_real_env.low_state_buffer.footForce)
               ...
               # =========== image send testing ==========
                ...
                if unitree_real_env.is_skill_mode():
                    ...
                    if not using_walk_policy:
                        ...
                        walk_model.reset()
                if not using_walk_policy:
                    # ============= using depth and kh_obs
                    # image = unitree_real_env.test()
                    # print("======= recieve image =====",image.shape)
                    
                    # obs = unitree_real_env._get_proprioception_kh_obs()
                    # print("============== recieve obs ============",obs.shape)

                    # depth_latent_yaw = depth_encoder(image, obs)
                    ...
                    # print("===depth_latent===",depth_latent_yaw)


                    obs[:,6:8] = 1.5*yaw


                    actions =depth_actor(obs, hist_encoding=True, scandots_latent= depth_latent).cpu()
                    
                    print( "=================== action computed!================")
                    print(actions)
                else:
                    walk_obs = unitree_real_env._get_proprioception_obs()
                   # print("=========== standing skill")
